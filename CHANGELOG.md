...
## v0.3.5 — 2026-09-25（UTC）

### 修复

- **对话卡住。** 流式请求此前把整个上游响应读完、解析、再合成完整事件序列一次性返回。客户端在这期间**收不到任何字节**，长回答下等待可达 75 秒，并常在 60 秒左右被客户端主动放弃（日志里的 HTTP 499）。

  实测数据：一次「从 1 数到 20」的流式请求，上游返回 **298 KB / 47 个事件**，全部收完需要 6.4-10.9 秒；真实对话生成更长。

  现在改为**异步流式转发**：
  - 先把上游连接建立起来，401/403/429 这类状态仍在本调用内同步返回，宿主据此冷却账号并换下一个；
  - 之后正文事件逐条通过 `host.stream.emit` 立即推送，客户端边生成边显示；
  - 与工具调用相关的事件（`function_call` / `custom_tool_call` / `output_item`）延后到流结束，用完整 item 还原成客户端工具调用后补发。

  这样既消除了无输出的等待，又保住了工具偷渡方案对完整 item 的依赖。

### 说明

- 上游首包时间实测约 11 秒（推理延迟），这部分等待无法通过实现缩短；改进的是首包之后的体验——从「全程无输出」变为「边生成边显示」。
- 测试新增环境变量隔离：代理相关环境变量会被 `normalize` 用来启用代理链，此前会让本地测试误走真实代理。

## v0.3.4 — 2026-09-25（UTC）

### 修复

- **模型列表只显示一个模型。** 默认只注册一个别名（`gpt-5.6-luna-basispoints`），用户在账号的模型列表里只看到它。

  实测结论（同一插件、两个不同档位的账号）：上游接受任意模型名，但**按账号权限路由到实际可用的模型**——免费档回显 `gpt-5.6-luna`，Plus 档回显 `gpt-5.6-sol`。也就是说「请求名」与「实际模型」不是一回事。

  现在默认注册一组常见别名（luna / sol / gpt-6），请求任一个都会由上游按账号权限落到真正可用的模型上。

- **保存代理配置时报 `Failed to fetch` 且无从排查。** 该错误是浏览器在网络层抛出的（请求根本没发出去），但页面把它和 HTTP 错误混为一谈，提示为"保存失败"。

  现在把三类失败分开并给出可操作提示：
  - 网络层失败 → 提示检查同源、CPA 是否运行、是否有反代或浏览器插件拦截
  - 401 / 403 → 提示密钥未填或密钥类型不对（管理密钥 vs 客户端 API Key）
  - 404 → 提示 CPA 版本不支持该端点或插件未加载
  - 响应非 JSON → 提示请求未命中管理接口

- 管理密钥自动读取增强：在 `cli-proxy-auth` 内按已知字段名查找后，会退化为有限深度扫描，兼容不同版本的字段命名差异。

### 说明

- 新增 `TestLiveModelCatalog`：给定令牌即可探测该账号实际可用的模型，便于按账号配置别名。

## v0.3.3 — 2026-09-25（UTC）

### 修复

- **默认配置里塞了开发机的本地代理，直连环境全部请求失败。** 插件此前把 `proxy_chain.enabled` 默认为 `true`，并把 `local_proxy` 默认为 `http://127.0.0.1:15732`。那只是开发机上用于联调的本地代理端口，部署到能直连上游的服务器后，每个请求都会先连一个不存在的本地端口。

  现在默认**直连**，不预置任何代理地址。代理是可选能力，只有确实需要经代理出网时才配置。

- **「只配远程代理」这种形态无法使用。** `newChainDialer` 会先解析 `local_proxy`，空地址直接报错，导致「服务器只填一个远程代理」这种最自然的配置根本用不了。现在本地跳与远程跳都可选，支持三种形态：
  - 直连（默认，不配置）
  - 单个代理（只填 `local_proxy` 或只填 `remote_proxy`）
  - 两级隧道（两者都填）

- 填写任一地址会自动视为启用；启用但没有任何地址会在配置校验阶段直接报错，避免「以为在用代理，其实直连」——那会让出口 IP 与预期不符，账号更容易被关联风控。

- 账号级代理（代理池）在全局直连时也能生效：此前它只替换远程段，全局关闭时代理会失效；现在指定账号代理会把代理链一并打开。

### 说明

- 管理界面的配置字段说明与示例配置不再出现任何具体调试端口。
- 新增测试覆盖三种代理形态、默认直连、以及「默认配置不得包含调试端口」的防回归检查。

...
打开 `/v0/resource/plugins/gpt365/panel`，「代理池」区块即可增删出口、切换策略、开关会话粘性，保存后**由宿主写盘并热重载**，无需重启。

页面上的口令处理：接口只回传主机名，**不回传口令**。编辑已有条目时地址框留空即表示保持原值；要更换地址就整条重填。
...
改动配置后，用页面上的「重新分配出口」让既有账号立刻采用新分配，不必逐个重新导入。
